...
```{r}
#Will change all defensive stats to numeric
defClean <- defClean %>%
        mutate(across(c("Tackles_Solo", "Tackles_ASST", "Tackles_TFL", "Tackles_Sack", "Turnover_FF", "Turnover_FR", "Turnover_INT", "Pass_QBH", "Pass_PB"))) %>%
        mutate_if(is.character, as.numeric, na.rm = TRUE) #turning all cols in defClean to numeric
str(defClean)     


#Will change all offensive stats to numeric
offClean <- offClean %>% 
                replace(is.na(.), 0) #replacing NAs with 0s
offClean <- offClean %>%
        mutate(across(c("Rushing_ATT", "Rushing_YDS", "Rushing_TD", "Receiving_REC", "Receiving_YDS", "Receiving_TD",    "Passing_CMP-ATT", "Passing_YDS", "Passing_TD", "Passing_INT", ))) %>%
        mutate_if(is.character, as.numeric, na.rm = TRUE) #turning all the columns in offClean to numeric
str(offClean)
